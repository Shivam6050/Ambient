...
function App() {
  ...
  return <main>
    ...
    <section className="card section"><h2>Decision timeline</h2>{!events.length && <p className="muted">Start a meeting, then simulate a delivery to see Ambient decide.</p>}{events.map(item => <article className="history" key={item.id}><h3>{title(item)} <span className="badge">{item.status}</span></h3><ol>{item.timeline.map((step, index) => <li key={index}><time>{time(step.at)}</time><strong>{step.action}</strong><span>{step.reason}{step.source && <small className="decision-source">{step.source === 'ai' ? 'AI proposal accepted' : step.source === 'fallback' ? 'Rules fallback' : step.source === 'policy' ? 'Policy enforced' : 'Rules'} · {step.policyReason?.replaceAll('_', ' ')}</small>}</span></li>)}</ol></article>)}</section>
    <p className="muted section">Saved in MongoDB: context, decisions, and dismissals survive server restarts.</p>
  </main>;
}
